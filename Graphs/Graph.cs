...
namespace Graphs
{
    ...
    {
        private readonly Dictionary<(int, int), int> _edgeWeights = new Dictionary<(int, int), int>();
        ...
        public IEnumerable<int> GetNeighbors(int i) => _adjacencyList[i];

        public int GetEdgeWeight(int startVertex, int endVertex) =>
            _edgeWeights.TryGetValue((startVertex, endVertex), out int weight) ? weight : 0;
        ...
        public (int Distance, List<int> Path) DijkstraShortestPath(int startVertex, int endVertex) =>
        ...
        {
            Dictionary<int, (int Distance, List<int> Path)> distances = new Dictionary<int, (int Distance, List<int> Path)>();
